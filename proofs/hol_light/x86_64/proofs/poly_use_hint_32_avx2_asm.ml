...
let poly_use_hint_32_avx2_asm_mc = define_assert_from_elf
 "poly_use_hint_32_avx2_asm_mc" "x86_64/mldsa/poly_use_hint_32_avx2_asm.o"
(*** BYTECODE START ***)
[
...
let POLY_USE_HINT_32_AVX2_ASM_BLOCK_CORRECT = prove
 (`!a h xb yb pc.
    ...
    ==> ensures x86
          (\s. bytes_loaded s (word pc) (BUTLAST poly_use_hint_32_avx2_asm_tmc) /\
               read RIP s = word pc /\
               C_ARGUMENTS [a; h] s /\
...
let POLY_USE_HINT_32_AVX2_ASM_NOIBT_SUBROUTINE_CORRECT = prove
 (`!a h x y pc stackpointer returnaddress.
    ...
    ==> ensures x86
          (\s. bytes_loaded s (word pc) poly_use_hint_32_avx2_asm_tmc /\
               read RIP s = word pc /\
...
let POLY_USE_HINT_32_AVX2_ASM_SUBROUTINE_CORRECT = prove
 (`!a h x y pc stackpointer returnaddress.
    ...
    nonoverlapping (stackpointer, 8) (a, 1024)
    ==> ensures x86
          (\s. bytes_loaded s (word pc) poly_use_hint_32_avx2_asm_mc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               C_ARGUMENTS [a; h] s /\
...
let POLY_USE_HINT_32_AVX2_ASM_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e a h pc stackpointer returnaddress.
          ...
          ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) poly_use_hint_32_avx2_asm_tmc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [a; h] s /\
...
let POLY_USE_HINT_32_AVX2_ASM_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e a h pc stackpointer returnaddress.
          ...
          ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) poly_use_hint_32_avx2_asm_mc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [a; h] s /\
