...
let poly_use_hint_88_avx2_asm_mc = define_assert_from_elf
 "poly_use_hint_88_avx2_asm_mc" "x86_64/mldsa/poly_use_hint_88_avx2_asm.o"
(*** BYTECODE START ***)
[
...
let poly_use_hint_88_avx2_asm_tmc =
  define_trimmed "poly_use_hint_88_avx2_asm_tmc" poly_use_hint_88_avx2_asm_mc;;

let POLY_USE_HINT_88_AVX2_ASM_EXEC =
...
(* Barrett model and the pure num/int decompose lemmas are arch-independent   *)
(* (shared with the AArch64-88 proof); the x86 lane decomposition mirrors the *)
(* poly_use_hint_32 proof with the 88 constants.                              *)
(* ========================================================================= *)
...
let POLY_USE_HINT_88_AVX2_ASM_BODY_BLOCK_TAC : tactic =
  ...
  SUBGOAL_THEN
   ...
   [REPEAT STRIP_TAC THEN
    ...
    ANTS_TAC THENL [ASM_ARITH_TAC; DISCH_THEN ACCEPT_TAC]; ALL_TAC] THEN
  EVERY (map (fun n -> X86_STEPS_TAC POLY_USE_HINT_88_AVX2_ASM_EXEC [n] THEN SIMD_SIMPLIFY_TAC[] THEN ABBREV_BIG_TAC) (1--28)) THEN
...
let POLY_USE_HINT_88_AVX2_ASM_BLOCK_CORRECT = prove
 (`!a h xb yb pc.
  ...
  REWRITE_TAC[MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI; C_ARGUMENTS; NONOVERLAPPING_CLAUSES; ALL;
              fst POLY_USE_HINT_88_AVX2_ASM_EXEC] THEN
  ...
  [
   ...
   ENSURES_INIT_TAC "s0" THEN
   X86_STEPS_TAC POLY_USE_HINT_88_AVX2_ASM_EXEC (1--17) THEN
   ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
   REWRITE_TAC DUPLITS_88 THEN
   REWRITE_TAC[ARITH_RULE `b < 0 <=> F`; LE_0] THEN ASM_REWRITE_TAC[]
  ;
   POLY_USE_HINT_88_AVX2_ASM_BODY_BLOCK_TAC
  ;
   REPEAT STRIP_TAC THEN X86_SIM_TAC POLY_USE_HINT_88_AVX2_ASM_EXEC (1--1)
  ;
   REWRITE_TAC[ARITH_RULE `32 <= b /\ b < 32 <=> F`] THEN
   ENSURES_INIT_TAC "s0" THEN
   X86_STEPS_TAC POLY_USE_HINT_88_AVX2_ASM_EXEC (1--1) THEN
...
let POLY_USE_HINT_88_AVX2_ASM_CORRECT = prove
 (`!a h x y pc.
    aligned 32 a /\ aligned 32 h /\
    nonoverlapping (word pc, 0xdd) (a, 1024) /\ nonoverlapping (a, 1024) (h, 1024)
    ==> ensures x86
  ...
  [FIRST_X_ASSUM(CONJUNCTS_THEN ASSUME_TAC) THEN
   ...
   ;
    ...
    ;
     MATCH_MP_TAC POLY_USE_HINT_88_AVX2_ASM_BLOCK_CORRECT THEN
...
let POLY_USE_HINT_88_AVX2_ASM_NOIBT_SUBROUTINE_CORRECT = prove
 (`!a h x y pc stackpointer returnaddress.
    aligned 32 a /\ aligned 32 h /\
    nonoverlapping (word pc, LENGTH poly_use_hint_88_avx2_asm_tmc) (a, 1024) /\
    nonoverlapping (a, 1024) (h, 1024) /\
    nonoverlapping (stackpointer, 8) (a, 1024)
    ==> ensures x86
          (\s. bytes_loaded s (word pc) poly_use_hint_88_avx2_asm_tmc /\
               read RIP s = word pc /\
  ...
  X86_PROMOTE_RETURN_NOSTACK_TAC poly_use_hint_88_avx2_asm_tmc POLY_USE_HINT_88_AVX2_ASM_CORRECT);;

let POLY_USE_HINT_88_AVX2_ASM_SUBROUTINE_CORRECT = prove
 (`!a h x y pc stackpointer returnaddress.
    aligned 32 a /\ aligned 32 h /\
    nonoverlapping (word pc, LENGTH poly_use_hint_88_avx2_asm_mc) (a, 1024) /\
    nonoverlapping (a, 1024) (h, 1024) /\
    nonoverlapping (stackpointer, 8) (a, 1024)
    ==> ensures x86
          (\s. bytes_loaded s (word pc) poly_use_hint_88_avx2_asm_mc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               C_ARGUMENTS [a; h] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(a, 1024)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE POLY_USE_HINT_88_AVX2_ASM_NOIBT_SUBROUTINE_CORRECT));;
...
let full_spec,public_vars = mk_safety_spec
    ...
    (REWRITE_RULE[MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI; SOME_FLAGS]
       POLY_USE_HINT_88_AVX2_ASM_CORRECT)
    POLY_USE_HINT_88_AVX2_ASM_EXEC;;

let POLY_USE_HINT_88_AVX2_ASM_SAFE = time prove
 (full_spec,
...
let POLY_USE_HINT_88_AVX2_ASM_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e a h pc stackpointer returnaddress.
          ...
          ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) poly_use_hint_88_avx2_asm_tmc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [a; h] s /\
               ...
               (\s s'. true)`,
  X86_PROMOTE_RETURN_NOSTACK_TAC poly_use_hint_88_avx2_asm_tmc POLY_USE_HINT_88_AVX2_ASM_SAFE THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let POLY_USE_HINT_88_AVX2_ASM_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e a h pc stackpointer returnaddress.
          aligned 32 a /\ aligned 32 h /\
          nonoverlapping (word pc, LENGTH poly_use_hint_88_avx2_asm_mc) (a, 1024) /\
          ...
          ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) poly_use_hint_88_avx2_asm_mc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [a; h] s /\
               ...
               (\s. read RIP s = returnaddress /\
                    ...
                    (exists e2.
                         ...
                         memaccess_inbounds e2 [a,1024; h,1024; stackpointer,8]
                                               [a,1024; stackpointer,8]))
               (\s s'. true)`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE POLY_USE_HINT_88_AVX2_ASM_NOIBT_SUBROUTINE_SAFE));;
